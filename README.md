...
*   **데이터 사각지대 해소:** 웹 크롤링이 불가능한 '학칙 및 규정' 파일의 텍스트를 추출하는 전처리 엔진 개발을 넘어, 교직원 연락처 정보와 관리자가 직접 지식(FAQ, 이벤트, 공지사항 등)을 등록하고 승인하는 시스템을 도입하여 정보의 사각지대를 해소합니다. 이로써 내규 정보 뿐만 아니라 다양한 최신 학내 정보까지 대화를 통해 습득할 수 있도록 합니다.

#### 1.3 개발 목표
*   **Advanced RAG 검색 구현:** LLM Router를 통해 질문 의도를 분석하고, Hybrid Search와 Re-ranking 과정에서 학과별 필터링, 날짜 기반 필터링 및 최신성 가중치 알고리즘을 적용하여 검색 정확도를 극대화합니다.
*   **신뢰성 있는 답변 생성:** 답변 생성 시 반드시 정보의 출처를 명시하고, 불필요한 서식(볼드체 등)을 제거하여 가독성과 정보의 신뢰성을 확보합니다.
*   **연속 대화 지원:** 이전 대화 맥락을 기억하는 멀티턴 기능을 구현하고, 현재 날짜 정보를 LLM에 제공하여 시의적절한 답변을 할 수 있도록 지원하며 자연스러운 사용자 경험을 제공합니다.

### 2. 최종결과물 소개

*   **Renux RAG 챗봇 시스템 (동똑이)**

    *   **기능:** 동국대학교 공지사항, 내규, 교직원 연락처, 전공 과목, 학사일정 및 관리자가 승인한 맞춤형 지식(FAQ, 이벤트, 공지사항)에 대한 질문에 대해 신뢰도 높은 답변 제공.
...
#### 3.1 프로젝트 진행과정
*   **아이디어 및 기획:** 단순 키워드 검색을 넘어 자연어 질문에 출처와 함께 답변하는 서비스 기획 및 관리자 지식 등록/승인 시스템 추가.
*   **데이터 구축:** 
    *   학교 공지사항, 학사 일정 크롤러 개발 및 교직원 연락처 데이터 수집.
    *   **자체 개발 전처리 엔진:** 접근이 어려운 내규 파일 텍스트 추출 및 정제.
    *   관리자 등록 지식(FAQ, 이벤트, 공지사항)을 위한 데이터 파이프라인 구축.
...
*   **AI 엔진 고도화:**
    ...
    *   **Re-ranking:** 최신성 가중치 알고리즘, 학과별 및 날짜별 필터링을 적용하여 검색 정확도 및 시의적절한 정보 우선 노출.
...
| :---: | :---: |
| <img src="doc/images/시퀀스 다이어그램-메인 요청_응답 흐름.png" width="100%"> | <img src="doc/images/시퀀스 다이어그램-RAG 서비스 내부 로직.png" width="100%"> |
...
1.  **데이터 수집/전처리:** Selenium/BeautifulSoup 크롤링 및 HWP 텍스트 추출, 교직원 연락처 수집, 관리자 등록 지식 -> 청킹(Chunking) -> 메타데이터 태깅.
...
5.  **생성(Generation):** 검색된 청크와 제약 조건(출처 표기, 현재 날짜 정보 전달 등)을 프롬프트에 주입하여 답변 생성 후, 불필요한 서식(볼드체) 제거.
