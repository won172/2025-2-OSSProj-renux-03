...
def test_점수는_0과_1_사이로_정규화된다(db):
    index = _build(db, "notices", {
        f"c{i}": f"장학금 신청 안내 {i}" for i in range(5)
    })
    점수 = index.score("장학금 신청")
    assert 점수.min() >= 0.0
    assert 점수.max() == pytest.approx(1.0)
